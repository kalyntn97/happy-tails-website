...
export default function Page() {
  ...
  return (
    <motion.div variants={containerVariants} initial='hidden' animate='visible' className="w-full flex flex-col lg:flex-row items-center justify-center px-5 pb-20 sm:px-20 lg:py-10">
      <motion.div variants={itemVariants} className="flex flex-col w-full lg:w-2/5 mb-10 lg:mr-20 items-center justify-center">
        ...
        <div>
          <h2 className="text-base sm:text-xl mb-5">Get in touch with me!</h2>
          <ContactLinks />
        </div>
      </motion.div>

      <motion.form variants={itemVariants} onSubmit={handleSubmit} ref={form} className="relative flex flex-col w-full lg:w-1/2 xl:w-1/4 sm:mt-10">
        ...
        <textarea required name="message" id='message' value={formData.message} onChange={(e) => handleChange(e)} className="rounded-sm p-2 my-2 h-32 border bg-stone-50 dark:bg-stone-800" placeholder="Message"/>
