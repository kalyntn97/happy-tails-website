...
export default function Page() {
  ...
  return (
    <main className="w-full flex flex-col sm:flex-row px-20 sm:items-center justify-center sm:justify-between">
      <div className="flex flex-col items-center">
        <div className="w-1/2 h-auto hidden sm:block">
          <Lottie animationData={dogAnimation} loop={true} />
        </div>
        <h1 className="text-2xl sm:text-5xl font-bold mt-10 mb-2">We'd love to hear from you</h1>
        <h4 className="text-base sm:text-xl font-bold text-stone-400">Got any questions, feedback, or review? Send us a message!</h4>
      </div>
      <form onSubmit={handleSubmit} ref={form} className="flex flex-col sm:w-1/2 sm:mt-40 mt-10 bg-red-100 p-10 rounded-lg">
        ...
        <button type='submit' className="group flex flex-row w-36 h-12 mt-10 bg-blue-300 items-center justify-center self-center rounded-lg hover:bg-blue-500 transition ease-in-out duration-300 delay-75 hover:-translate-y-1">
          <Image src={'/icons/send.png'} alt='a paperplane icon' width={32} height={32} className="mr-4 group-hover:rotate-45 group-hover:scale-110 duration-500" />
          <span className="group-hover:hidden duration-500 font-bold">SUBMIT</span>
        </button>
      </form>
    </main>
  );
